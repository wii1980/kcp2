...
#   ./build.sh --aead       # 启用 AEAD 加密
# =============================================================================

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
EXAMPLE_NAME="embassy-esp32-heartbeat"
...
while [[ $# -gt 0 ]]; do
	...
	--help | -h)
		...
		echo "  --aead        启用 AEAD 加密 (ChaCha20-Poly1305)"
		echo "  --aes         启用 AEAD 加密 (AES-256-GCM，需同时启用 aead)"
		echo "  -h, --help    显示帮助"
		exit 0
		;;
	*)
...
if [ "$AEAD" = true ]; then
	if [ "$AES" = true ]; then
		echo -e "${CYAN}  AEAD: AES-256-GCM${NC}"
	else
		echo -e "${CYAN}  AEAD: ChaCha20-Poly1305${NC}"
	fi
fi
echo -e "${CYAN}========================================${NC}"
...
if [ "$AEAD" = true ]; then
	BUILD_ARGS+=(--features aead)
fi

if [ "$AES" = true ]; then
	BUILD_ARGS+=(--features aead-aes)
fi
